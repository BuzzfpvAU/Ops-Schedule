...
beforeEach(() => {
  ...
  db.prepare("INSERT INTO jobs (id, code, name) VALUES ('j1', 'T1', 'Transit job')").run();
...
test('migration adds transit columns with 0 defaults', () => {
  const cols = db.pragma('table_info(job_equipment)').map(c => c.name);
  assert.ok(cols.includes('transit_before'), 'transit_before exists');
  assert.ok(cols.includes('transit_after'), 'transit_after exists');
});

test('migration is idempotent: re-init keeps assignments and transit', async () => {
  const a = await assign();
  assert.equal(a.status, 201);
  await call('PUT', `/equipment/${a.body.id}`, { transit_before: 120, transit_after: 45 });
  ...
  assert.equal(card.equipment[0].transit_before, 120);
  assert.equal(card.equipment[0].transit_after, 45);
});

test('admin sets transit and the job card returns it', async () => {
  const a = await assign();
  const put = await call('PUT', `/equipment/${a.body.id}`, { transit_before: 120, transit_after: 45 });
...
test('transit validation rejects negatives, fractions and unknown rows', async () => {
  const a = await assign();
  for (const body of [{ transit_before: -5 }, { transit_after: 1.5 }, { transit_before: 'x' }]) {
    const res = await call('PUT', `/equipment/${a.body.id}`, body);
    assert.equal(res.status, 400, JSON.stringify(body));
  }
  const missing = await call('PUT', '/equipment/nope', { transit_before: 10 });
  assert.equal(missing.status, 404);

  const row = db.prepare('SELECT transit_before, transit_after FROM job_equipment WHERE id = ?').get(a.body.id);
  assert.equal(row.transit_before, 0);
  assert.equal(row.transit_after, 0);
});

test('non-admin cannot set transit', async () => {
  const a = await assign();
  currentUser = { memberId: 'm1', isAdmin: false, isViewer: false };
  const res = await call('PUT', `/equipment/${a.body.id}`, { transit_before: 30 });
